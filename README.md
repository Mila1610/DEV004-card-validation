...
* [3. Pistas, tips y lecturas complementarias](#3-Pistas, tips y lecturas complementarias)
...
* `README.md`: explica la información necesaria para el uso de la aplicación
  ...
  diseño que tome.
* `src/index.html`: este es el punto de entrada de mi aplicación. Este archivo
  contiene nuestro input y el boton para los datos requeridos e incluye el CSS y JavaScript.
* `src/style.css`: este archivo contiene las reglas de estilo y nuestro diseño perzonalizado.  
  La cual a sido pensado en un restaurante la cual tiene que válidar nuestro número de tarjeta y ver a que franquisia pertenece.
* `src/validator.js`: contiene el objeto `validator`. Este objeto (`validator`) debe contiene tres
  métodos:
  ...
  - `maskify(creditCardNumber)`: `creditCardNumber` es un `string` con
    ...
    maskify(               '')  ===                ''
    ```
  - `franquisia(creditCardNumber)`: `creditCardNumber` es un `string`
     con el número de tarjeta que se va a verificar si pertenece a un banco. Esta función debe retornar
     a que banco pertenece de acuerdo a la [franquisia](https://stevemorse.org/ssn/cc.html).

* `src/index.js`: escucha los eventos del DOM, invoca
  `isValid()`, `maskify()` y `franquisia()` y
  actualiza el resultado en la UI (interfaz de usuario).

## 3. Pistas, tips y lecturas complementarias
